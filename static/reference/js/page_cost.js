...
/* ------------------------parsing (end)--------------------------- */
    ...
    document.getElementById("menu1.2").classList.add("current");
    ...
    var columnlist = [];
    for (var i = 0; itemscost.length > i; i++) {
         var currentitem = itemscost[i];
         for (res in currentitem.cost) {
    ...
    for (var i = 0; itemscost.length > i; i++) {
        ...
        for (var j = 0; columnlist.length > j; j++) {
            ...
            var temp_val = currentitem.cost[temp_res];
            if (temp_val) {
