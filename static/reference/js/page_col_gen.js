...
/* ------------------------parsing (end)--------------------------- */
    ...
    document.getElementById("menu4.1").classList.add("current");
    ...
    // build column list
    console.log(items);
    var columnlist = ['Name', 'Avg per tick','Avg per min', 'Compared to Stargate', 'colGenRate', 'touristDoorway', 'isStargate', 'Details'];
    ...
    // get stargate rate
    var stargaterate = null;
    for (var i = 0; items.length > i; i++) {
        var currentitem = items[i];
        if (currentitem.name=='Star Gate') {
            stargaterate = currentitem.colgen;
        }
    }
    if (stargaterate==null) {
    ...
    for (var i = 0; items.length > i; i++) {
        var currentitem = items[i];
        if (currentitem.colgen>0) {
            if (currentitem.requiPremium) {
                var temp_p = '1';
            } else {
                var temp_p = '0';
            }
            htmltablebody += '<tr><td>'+currentitem.name+'|'+currentitem.transModeColor+'|'+temp_p+'</td>'; //name
            htmltablebody += '<td>'+(currentitem.colgen).toFixed(4)+'</td>'; // true colgen rate
            htmltablebody += '<td>'+(currentitem.colgen*30*60).toFixed(3)+'</td>'; // col per min
            htmltablebody += '<td>'+(currentitem.colgen/stargaterate).toFixed(3)+'</td>'; // stargate
            htmltablebody += '<td>'+currentitem.colGenRate+'</td>'; // colGenRate
            htmltablebody += '<td>'+currentitem.touristDoorway+'</td>'; // touristDoorway
            htmltablebody += '<td>'+currentitem.isStargate+'</td>'; // isStargate
            htmltablebody += '<td>'+currentitem.className+'</td></tr>'; // details
        }
    }
