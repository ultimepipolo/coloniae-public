...
/* ------------------------parsing (end)--------------------------- */
    ...
    document.getElementById("menu1.3").classList.add("current");
    ...
    var columnlist = [];
    for (var i = 0; itemsstore.length > i; i++) {
         var currentitem = itemsstore[i];
         for (res in currentitem.storesObj) {
    ...
    for (var i = 0; itemsstore.length > i; i++) {
        ...
        for (var j = 0; columnlist.length > j; j++) {
            ...
            var temp_val = currentitem.storesObj[temp_res];
            if (temp_val) {
                htmltablebody += '<td>'+temp_val.toLocaleString()+'</td>';
            } else {
                htmltablebody += '<td></td>';
            }
        }
